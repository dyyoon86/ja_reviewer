...
def regen_narration(folder: Path, meta_api: str, log=print, seq=None, slots=None):
    ...
    ROLE_DESC = {
        ...
        "컨셉":   ("역할=설정 한 줄 + **스테이크**. [줄거리]와 장르 태그를 근거로 어떤 설정인지 밝히고, "
                  "그게 왜 곤란한/위험한 상황인지까지 한 문장에 담는다. "
    ...
    ROLE_DESC["소개컨셉"] = ("역할=소개+설정. '○ 번째 작품은 {a}입니다' 뒤에 어떤 설정이고 왜 곤란한지를 "
                          "한 문장으로 붙인다.").format(a=actress)
...
{fact_block}[줄거리 — ★참고자료가 아니라 필수 포함 항목이다]
