...
def _style_jindong():
    """진동기형 — 배우 '스펙 브리핑' 정보 리뷰.

    근거: @진동기-74 영상 오디오를 large-v3로 직접 전사(2026-09-07, 2편) + 자동자막 6편.
      자동자막은 '제유 업체'(=제휴), '품뻔'(=품번)처럼 오인식이 심해 문체 판단에 못 쓴다.

    다른 문체가 '줄거리'를 말할 때 이 채널은 **배우의 스펙과 소속 맥락**을 말한다.
    60초 안에 데뷔 시기 → 외모·피지컬 → 소속사/제작사 평가 → 조건부 추천이 다 돈다.
    핵심은 **냉정함**이다. 단점을 먼저 말하고 장점으로 덮는다("연기력이 엄청나게 좋은
    편은 아니지만 매우 예쁜 얼굴과 꿀피부로 사랑받는 A급"). 제작사도 깐다
    ("평소 촬영을 정말 개떡같이 하는 제작사인데 이 작품에서는 그래도 좀 괜찮게 나왔습니다").
    """
    return (
        "[캐릭터] 딸딸기튜브 리뷰어 '딸감별사'. 진동기 채널 문체 — "
        "줄거리보다 **배우 스펙과 제작 맥락**을 브리핑하는 정보형 리뷰어.\n"
        "[★핵심 — 냉정한 스펙 브리핑] 이 문체의 중심은 '이 배우가 어떤 스펙이고 "
        "이 작품이 그 스펙을 살렸는가'다. 데뷔 시기·레이블·외모/피지컬·이번 작품의 위치를 "
        "메타에서 끌어낼 수 있는 사실로만 엮는다.\n"
        "[★단점 먼저] 띄워주기로 시작하지 않는다. **부족한 점을 먼저 말하고 장점으로 덮는** "
        "역접 구조를 쓴다 — '~은 아니지만 ~로 사랑받는'. 제작사·레이블도 평가 대상이다.\n"
        "[어조] 정중체(~입니다/~인데요). 담백하고 과장이 없다. 느낌표 금지. "
        "'미쳤습니다' 대신 '꽤 괜찮습니다', '좀 아쉽네요' 정도로 절제한다.\n"
        "[연결어미] '~인데요'로 이어가다 '~습니다'로 끊는 리듬. 한 문장 20~30자.\n"
        "[사견] '개인적으로는 ~인 것 같습니다', '~면 참 좋겠네요' 같은 주관을 1~2회.\n"
        "[배우 호칭] 이름 뒤에 '장/짱' 또는 '님'(예: '유카짱', '소라님').\n"
        "[어휘] 명실상부·A급·메이저·신인·데뷔작·해금작·필모·피지컬·꿀피부·골반·"
        "미드·캐미·폼·연기력·순급·원툴·국밥 조합.\n"
        "[문장 예시 — 실제 대본에서 뽑은 것. 이 톤을 그대로 따를 것]\n"
        "· 오늘의 주인공은 25년 5월에 데뷔한 배우인데요.\n"
        "· 연기력이 엄청나게 좋은 편은 아니지만 예쁜 얼굴과 꿀피부로 사랑받는 A급이죠.\n"
        "· 다만 소속사가 좀 재미없기로 유명한 곳이라 아직 끼를 다 발산하지 못합니다.\n"
        "· 솔직히 이번 작품은 외모 원툴입니다.\n"
        "· 하지만 이 조합이 또 가장 수요가 많은 국밥 조합이죠.\n"
        "· 외모는 상위권이지만 발랄함이나 표현력은 좀 부족합니다.\n"
        "· 시간이 지나면 더 성장할 수 있는 배우라고 생각합니다.\n"
        "· 얼굴을 중요시하는 분에게는 참 좋은 작품이 될 것 같은데요.\n"
        "[마무리] 배우를 한 마디로 수식하며 배웅하듯 닫는다 — "
        "'오늘도 수요가 많은 그녀였습니다.', '오늘도 A급 배우 ○○짱이었습니다.' 식. "
        "채널 인사·구독·제휴 유도는 넣지 않는다(모음집 맨 끝에서 사람이 붙인다).\n")


def _style_naeson():
    """내손내싼형 — 1인칭 리액션 개그 리뷰.

    근거: @내손내싼 영상 오디오를 large-v3로 직접 전사(2026-09-07, 4편)
      + 벤치마킹/내손내싼/분석.md(조회수 상위 5편 정독).

    작품 정보가 아니라 **'내가 보다가 어떻게 됐는지'**가 콘텐츠다. 첫 문장이 이미
    결론 리액션이라 3초 안에 후킹된다("이 여인과 함께한다면 하루도 피곤하지 않을 날이
    없겠다 싶었습니다").
    시청자를 '형님들'로 부르고, 게임 스탯·분류학·계절 같은 **엉뚱한 프레임**을 끌어와
    작품을 설명하는 게 이 채널 유머의 본체다("모든 스탯이 얼굴에 몰빵되어 있는 타입").
    격식체 존댓말인데 내용이 코믹해서 생기는 낙차가 웃음 포인트 — 톤을 올리면 망한다.
    """
    return (
        "[캐릭터] 딸딸기튜브 리뷰어 '딸감별사'. 내손내싼 채널 문체 — "
        "**내가 직접 보고 나서 어떻게 됐는지**를 말하는 1인칭 리액션 리뷰어. "
        "잘난 척이 아니라 절박한 자학 페르소나라 비호감이 없다.\n"
        "[★구조 — 이 순서를 지킬 것]\n"
        " ① **결론 리액션 먼저** — 배우 소개보다 내 반응이 앞선다. "
        "'이번 작품은 ~입니다'로 열면 이 문체가 아니다.\n"
        " ② 엉뚱한 프레임 도입 — 게임 스탯·분류학·계절·일상 소재를 끌어와 작품을 설명한다.\n"
        " ③ 뒤집는 근거 — 구체적인 감상 경험 하나로 판을 뒤집는다.\n"
        " ④ 조건부 강권 — '~하시는 형님들은 무조건', '한 번만 보지 마시고 세 번은'.\n"
        "[어조] 격식 있는 존댓말(~습니다/~드립니다/~바랍니다)로 코믹한 내용을 말한다. "
        "**이 낙차가 웃음 포인트다 — 톤을 올려 웃기려 하면 망한다.** 느낌표 금지.\n"
        "[시청자 호칭] '형님들', '성님들'. 배우는 '누님', '이 여인'.\n"
        "[★엉뚱한 프레임] 편당 최소 1회. 작품과 무관한 체계를 끌어와 설명한다 — "
        "게임 능력치, 분류학, 계절, 음식. 이게 이 채널 유머의 본체다.\n"
        "[구체 비유 의무] 추상 형용사('예쁜','매력적인') 단독 금지. 상황이 그려지는 비유로.\n"
        "[밈 인용] '얼굴이 개연성이다' 처럼 인터넷 밈을 근거인 척 인용하는 것 1회 허용.\n"
        "[문장 예시 — 실제 대본에서 뽑은 것. 이 톤을 그대로 따를 것]\n"
        "· 이 여인과 함께한다면 하루도 피곤하지 않을 날이 없겠다 싶었습니다.\n"
        "· 세상에는 다양한 빠들이 있습니다. 얼빠, 몸빠, 슴빠, 골반빠.\n"
        "· 게임 캐릭터 스탯을 찍듯 모든 능력치가 얼굴에 몰빵되어 있는 타입입니다.\n"
        "· 옷을 입고 계셨을 땐 티가 안 나지만 여러모로 아쉬운 점이 많습니다.\n"
        "· 무엇 하나 거를 타선이 없는 상당한 수작입니다.\n"
        "· 형님들의 머리채라도 잡고 강제로 시청하게 하고 싶은 영상이었습니다.\n"
        "· 얼굴이 개연성이다 라는 말도 있지 않습니까?\n"
        "· 한 번만 보지 마시고 세 번은 보시기 바랍니다.\n"
        "[마무리] 조건부 강권 또는 역추천 펀치라인으로 닫는다. "
        "채널 시그니처 인사('저란 남자는 ~입니다')·구독·링크 유도는 넣지 않는다.\n")


def _style(kind="3min"):
    """내레이션 문체 선택.
    3min=3분휴지형(총평 리뷰) / cinema=고몽·김시선형(대사 주고받기) /
    gootabari=구타바리형(반말 병맛 초압축 + 괄호 드립) /
    jindong=진동기형(배우 커리어 정보) / naeson=내손내싼형(1인칭 리액션 개그)."""
    if kind == "cinema":
        return _style_cinema()
    if kind == "gootabari":
        return _style_gootabari()
    if kind == "jindong":
        return _style_jindong() + _human_tone()
    if kind == "naeson":
        return _style_naeson() + _human_tone()
    return _style_3min()
...
def _must_have(kind="3min"):
    ...
    if kind == "jindong":
        return (
            "[내레이션 필수 요소 — 하나라도 빠지면 실패]\n"
            " (a) 전환+소개 한 문장 — 위 [묶음 리뷰] 지시대로 열고 배우를 호명한다.\n"
            " (b) ★배우 커리어 — 데뷔 시기·레이블·필모 흐름 중 **최소 2가지**를 엮는다."
            " 이 문체의 중심이라 빠지면 다른 채널과 구분이 안 된다.\n"
            " (c) 이번 작품이 그 흐름에서 어떤 위치인지 한 줄.\n"
            " (d) 사견 1~2회 — '개인적으로는 ~인 것 같습니다' 형태.\n"
            " (e) 아쉬운 점 최소 1개. 과열된 칭찬만 늘어놓지 않는다.\n"
            " (f) 마무리 — 배우를 한 마디로 수식하며 배웅하듯 닫는다.\n"
            "[자기점검] 출력 전에 확인: 느낌표·과장 감탄이 있는가? 커리어 정보가 2가지 이상인가? "
            "메타에 없는 사실을 지어내지 않았는가? 채널 인사·구독 요청을 넣지 않았는가?\n")
    if kind == "naeson":
        return (
            "[내레이션 필수 요소 — 하나라도 빠지면 실패]\n"
            " (a) ★첫 문장은 **결론 리액션** — 배우 소개보다 내 반응이 먼저다."
            " '이번 작품은 ~입니다'로 열면 이 문체가 아니다.\n"
            " (b) 원래 기대 한 줄 — '원래는 ~쪽으로 보려 했습니다만'.\n"
            " (c) 뒤집는 근거 — 구체적인 감상 경험 하나로 판을 뒤집는다.\n"
            " (d) 몸 리액션 평가 최소 1회 — 별점 대신 내 반응으로 등급을 매긴다.\n"
            " (e) 구체 비유 최소 1회 — 추상 형용사('예쁜','매력적인') 단독 사용 금지.\n"
            " (f) 마무리 — 조건부·역설 역추천 펀치라인으로 닫는다.\n"
            "[자기점검] 출력 전에 확인: 첫 문장이 소개체로 시작하지 않았는가? "
            "톤을 올려 웃기려 하지 않았는가(드라이해야 한다)? 셀프 츳코미가 2회 넘지 않는가? "
            "채널 시그니처 인사·구독 요청을 넣지 않았는가?\n")
    if kind == "cinema":
        return (
            "[내레이션 필수 요소 — 하나라도 빠지면 실패]\n"
...
def _style_3min():
    ...
    return (
        ...
        # ★2026-09-07 추가 — 벤치마킹/3분휴지/분석.md 재검토. 조회수 상위 영상(89만·75만)의
        #   핵심 장치인 '미스터리 구조'가 프롬프트에 빠져 있었다. 이 채널은 평범한 상황극도
        #   동기를 숨기고 "왜?"를 던져 추리물처럼 재포장한다 — 그게 체류시간을 만든다.
        "[★미스터리 구조 — 조회수 상위 영상의 실제 장치]\n"
        " · 인물의 **목적을 처음부터 밝히지 마라.** 관계만 먼저 세우고 동기는 숨긴다.\n"
        " · 질문형 떡밥을 2회 이상 심는다 — '왜 남편에게 거짓말을 하는 걸까요?', "
        "'다른 목적이 있었던 걸까요?'\n"
        " · 숨겨둔 동기는 **중반 이후**에 공개한다 — '사실 좋아했던 사람은 ~였습니다.'\n"
        " · 심리 해설을 섞는다 — '표정이 무언가 아쉬워하는 것 같습니다', '~인가 봅니다'. "
        "정적인 컷도 이 한 줄이 붙으면 서사가 생긴다.\n"
        " · **결말은 절대 밝히지 않는다.** 마지막 장면 줄은 '과연 ~일까요?'로 끊는다.\n"
        "[검열 회피 어휘 — 이 채널 실사용] 정사 장면=액션 신/플레이, 관계를 가지다=선을 넘다/"
        "불타오르다, 성인용품=전설의 아이템. 직접 표현 대신 이쪽을 쓴다.\n"
...
def _plain_korean():
    """일상어 강제 — 평론투 한자 추상명사를 막는다.

    사용자 지적(2026-09-07, FNS-253 줄거리): "비가 그친 뒤 찾아오는 어색한 침묵과
    뒤늦은 현실감" — '현실감' 같은 말은 일상에서 안 쓴다. LLM은 요약을 시키면
    자동으로 평론투로 흐르는데, 그 줄거리가 그대로 내레이션의 근거가 되므로
    여기서 막지 않으면 최종 대본까지 평론투가 번진다."""
    return (
        "[말투 — 일상어로 쓴다]\n"
        "· 평론투 한자 추상명사 금지: 현실감·긴장감·이질감·위화감·여운·정적·관능·"
        "서사·몰입감·개연성·카타르시스·심리 묘사.\n"
        "· '~감', '~성', '~적인'으로 끝나는 추상어를 쓰지 마라.\n"
        "· 뜻이 두루뭉술한 동사 금지: '흐려진다', '고조된다', '교차한다', '드러난다', "
        "'자아낸다'. **무슨 일이 있었는지**를 써라.\n"
        "· 추상명사를 'A와 B'로 나열하지 마라.\n"
        "  (나쁨: '어색한 침묵과 뒤늦은 현실감')\n"
        "  (좋음: '비가 그치고 나서야 둘 다 말이 없어진다. 그제야 정신이 든 얼굴.')\n"
        "· 친구한테 줄거리 설명하듯 써라. 평론가가 아니라 **본 사람이 말해 주는** 톤.\n")
...
# ★ 내레이션 밀도의 **단일 기준** — 여기 한 곳에서만 정한다.
# 2026-08-03 사용자 지시("내레이션 짧아도 된다") 이후 regen.narration_slots는 15초당 1줄
# (5~10줄)로 내려갔는데, 이 프롬프트의 예산은 옛 3분휴지 실측(5.4초당 1문장)을 그대로
# 써서 98초 영상에 "17~20문장"을 요구했다 — 두 값이 3배 어긋나 있었다.
# LLM은 어차피 그 수를 못 지켰고(ja20 실측 2~10줄), 뒤에서 regen이 7줄로 다시 짜니
# 프롬프트만 헛돌았다. 두 곳이 같은 함수를 쓰게 해서 어긋남 자체를 없앤다.
NAR_SEC_PER_LINE = 15.0    # 내레이션 한 줄이 맡는 영상 길이(초)
NAR_LINES_MIN, NAR_LINES_MAX = 5, 10
NAR_CHARS_PER_LINE = 24    # 슬롯 상한(regen.NAR_MAX_CHARS=30) 안쪽의 실질 평균


def narration_lines(video_sec, lo=NAR_LINES_MIN, hi=NAR_LINES_MAX, per=NAR_SEC_PER_LINE):
    """영상 길이 → 내레이션 줄 수. 프롬프트 예산(narration_budget)과 재생성 슬롯
    (regen.narration_slots)이 **함께** 쓰는 유일한 밀도 규칙.

    촘촘하게 채우면 쓸 말이 떨어져 같은 소재를 반복한다(ADN-795: 14줄 중 4줄이
...
def narration_budget(target_sec, style="3min"):
    ...
    3min : 15초당 1줄(5~10줄) — narration_lines와 동일 기준. 120초 = 7~9문장·192자
    ...
    else:                                  # 3min(기본) — 알 수 없는 값도 여기로
        n = narration_lines(target_sec)
        return max(3, n - 1), n + 1, n * NAR_CHARS_PER_LINE
...
def _roundup_block(pos="mid", target_sec=60, style="3min"):
    ...
    if pos == "solo":
        ...
        return ("[★단독 리뷰] 이 내레이션은 이 작품 하나로 완결되는 독립 영상이다(묶음 아님).\n"
                ...
                " · 마지막은 '~작품이었습니다.'로 총평을 닫고 끝낸다. "
                "마무리 인사('시청 감사', '지금까지 ~였습니다', '구독')는 넣지 않는다 — "
                "모음집 끝의 섹션4 아웃트로가 맡는다.\n")
    ...
    # 마무리 인사는 섹션4 아웃트로가 맡는다(regen.py와 같은 규칙) — 마지막 꼭지에도 넣지 않는다
    return b
...
def prompt_story(meta, segs, max_lines=None, site_desc=""):
    """원본 **전체** 전사 → '잘려나간 부분까지 포함한 줄거리 브리핑'.

    왜 필요한가: 섹션②는 클린본만 본다. 클린이 공격적일수록(ABF-382: 101분 → 1.6분)
    LLM은 전체의 2%만 보고 요약·컷선정·내레이션을 하게 된다 — 원본에 내용 대사가
    104줄 있어도 클린본엔 15~20줄뿐이라 이야기의 앞뒤가 사라진다.
    섹션①의 2️⃣ 소리 필터가 원본 전체를 이미 전사하므로(비용 0), 그 전사본으로
    줄거리를 먼저 세우고 그것을 컷 선정 프롬프트에 힌트로 넣는다.

    ★출력은 그대로 프롬프트에 들어가고 최종 내레이션의 근거가 되므로, 노골적 표현
      없이 '상황·관계·전환점'만 쓰게 한다(헤드리스 거부 방지 + 유튜브 안전)."""
    rows = segs[:max_lines]
    body = "\n".join(f"{a:.0f}\t{t}" for a, _b, t in rows)
    more = f"\n(이하 {len(segs) - len(rows)}줄 생략)" if len(segs) > len(rows) else ""
    # ★ 사이트 소개문과의 교차 검증 — 전사만 보고 쓴 줄거리는 STT 오인식·화자 혼동으로
    #   엉뚱한 이야기가 될 수 있다. 판매 사이트 소개문은 사람이 쓴 독립 근거이므로,
    #   둘을 맞춰 보면 "전사를 잘못 읽었는지"를 값싸게 알아낼 수 있다.
    cross = ""
    out_keys = '"story":["..."],"cast":["..."],"turns":["..."],"hooks":["..."]'
    if site_desc:
        cross = (f"\n[사이트 소개문 — 판매 페이지에 사람이 써 둔 것. 별개의 근거다]\n"
                 f"{site_desc}\n"
                 f"※ 노골적인 표현이 섞여 있을 수 있다. **판정에만** 쓰고 출력에는 "
                 f"그대로 옮기지 마라.\n")
        out_keys += ',"match":"일치|부분일치|불일치","match_note":"한 줄 근거"'
    check = ("" if not site_desc else
             " · match: 위 소개문과 네가 전사에서 읽어낸 이야기가 같은 작품인지 판정한다.\n"
             "   '일치'=핵심 설정이 같다 / '부분일치'=큰 틀은 맞으나 세부가 다르다 / "
             "'불일치'=다른 이야기로 보인다.\n"
             " · match_note: 무엇을 근거로 그렇게 봤는지 한 줄. 어긋나는 대목이 있으면 "
             "그것을 짚어라(전사 오인식일 수 있다).\n")
    return (f"너는 일본 영상의 줄거리를 정리하는 편집자다. 아래는 **원본 전체**의 "
            f"일본어 자막이다(시작초\\t대사). 신음·감탄은 이미 섞여 있으니 무시하고 "
            f"'이야기'만 읽어라.\n"
            f"[메타]\n{_meta_block(meta)}\n[원본 자막 전문]\n{body}{more}\n{cross}"
            f"[할 일] 이 작품이 어떤 이야기인지 정리한다. 뒤 단계에서 이 브리핑만 보고 "
            f"하이라이트를 고르고 해설 내레이션을 쓸 것이므로, **읽는 사람이 작품을 "
            f"안 봐도 흐름을 알 수 있게** 써라.\n"
            f" · story: 도입→관계→전환→결말 순서로 6~10줄. 각 줄 앞에 대략의 시각(분)을 붙인다.\n"
            f" · cast: 등장인물과 관계(이름이 안 나오면 '남자/여자/선배' 등 역할로).\n"
            f" · turns: 이야기가 꺾이는 지점 2~4개. 각각 '몇 분쯤, 무슨 일'.\n"
            f" · hooks: 리뷰 영상에 쓰면 시청자가 멈출 만한 순간 3~5개(한 줄씩).\n"
            f"{check}"
            f"{_plain_korean()}"
            f"[표현 규칙] 성적 묘사·신체 부위·행위는 쓰지 마라. 상황·감정·관계·긴장만 쓴다. "
            f"약물류는 '몽롱해지는 것'처럼 순화한다. **모든 출력은 한국어로.**\n"
            f'[출력 JSON만] {{{out_keys}}}')
